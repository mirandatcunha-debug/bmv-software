...
export default function ClientesPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/cadastros/clientes?${params.toString()}`)
        if (response.ok) {
  ...
  const handleDesativar = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/cadastros/clientes/${id}`, {
        method: 'DELETE',
      })
      if (response.ok) {
        fetchClientes()
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-6">
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 transition-all hover:bg-white/20 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs text-blue-200">Total Ativos</span>
              </div>
              <p className="text-base sm:text-lg font-bold">{clientesAtivos}</p>
            </div>
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 transition-all hover:bg-white/20 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs text-blue-200">Pessoa Fisica</span>
              </div>
              <p className="text-base sm:text-lg font-bold">{clientesPF}</p>
            </div>
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 transition-all hover:bg-white/20 animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs text-blue-200">Pessoa Juridica</span>
              </div>
              <p className="text-base sm:text-lg font-bold">{clientesPJ}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <CardContent className="p-4">
          <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex gap-2">
              ...
              <Button type="button" variant="outline" onClick={fetchClientes}>
              ...
              <CardContent className="pb-4">
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex items-center gap-2 text-muted-foreground text-xs pt-2 border-t">
                    ...
                    <span>Cadastrado em {formatDate(cliente.criadoEm)}</span>
                  </div>
                </div>

                {/* Acoes */}
                <div className="flex gap-2 mt-4 pt-3 border-t">
                  <Link href={`/cadastros/clientes/${cliente.id}`} className="flex-1">
                    <Button variant="outline" size="sm" className="w-full">
                      <Eye className="h-3.5 w-3.5 mr-1" />
                      Ver
                    </Button>
                  </Link>
                  <Link href={`/cadastros/clientes/${cliente.id}?edit=true`} className="flex-1">
                    <Button variant="outline" size="sm" className="w-full">
