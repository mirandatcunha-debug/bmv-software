...
// GET - Listar contas a pagar do tenant
export async function GET(request: NextRequest) {
  try {
    ...
    const whereClause: Prisma.PayableWhereInput = {
    ...
    const contas = await prisma.payable.findMany({
      where: whereClause,
    ...
    const contasFormatadas = contas.map(conta => {
      ...
      return {
        ...
        fornecedorId: conta.fornecedorId || conta.supplierId,
...
// POST - Criar nova conta a pagar com parcelas
export async function POST(request: NextRequest) {
  try {
    ...
    // Validacoes
    if (!fornecedorId || !descricao || !valorTotal || !primeiroVencimento) {
      return NextResponse.json(
        { error: 'Campos obrigatorios: fornecedor, descricao, valor e data de vencimento' },
        { status: 400 }
      )
    }
    ...
    const result = await prisma.payable.createMany({
      data: parcelasData,
    })
